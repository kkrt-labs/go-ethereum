...
// --- Start fork code ---
func TestVerifyProofWithProof(t *testing.T) {
	testCase := []struct {
		desc  string
		root  string
		key   string
	...
	}{
		{
			...
			expectedValue:           "0xf8440101a065d17ccfe8328a42712f5dcd7a8827eebab3341e1a8bd6a4cb741495b83bd026a0b44fb4e949d0f78f87f79ee46428f23a2a5713ce6fc6e0beb3dda78c2ac1ea55",
			expectedProofNodesCount: 8,
			expectedLastKey:         "ab14d68",
		...
		},
		{
			...
			expectedValue:           "0x",
			expectedProofNodesCount: 7,
			expectedLastKey:         "ab14d6",
	...
	for _, tc := range testCase {
		t.Run(tc.desc, func(t *testing.T) {
			...
			// Verify the proof
			proof, err := VerifyProofWithProof(common.HexToHash(tc.root), hexutil.MustDecode(tc.key), proofDB)
			if tc.expectedError != nil {
				...
				assert.Equal(t, tc.expectedValue, hexutil.Encode(proof.Value()), "Value mismatch")
				assert.Len(t, proof.Nodes(), tc.expectedProofNodesCount, "Invalid count of nodes")
				assert.Equal(t, tc.expectedLastKey, encodeKey(lastNode.Path), "Last key prefix mismatch")
				assert.Equal(t, tc.expectedLastNode, hexutil.Encode(lastNode.Node.Blob), "Last node mismatch")
			}
		})
	}
}
